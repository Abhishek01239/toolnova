...
export async function rankWithAI(candidates, site) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        if (attempt === maxAttempts) throw new Error('Groq HTTP 429 Rate Limit reached');
        const waitMs = parseResetTime(res) + 2000;
        console.warn(`⏳ Groq ranking HTTP 429 Rate Limit. Retrying in ${(waitMs / 1000).toFixed(1)}s (attempt ${attempt}/${maxAttempts})...`);
        clearTimeout(timer);
