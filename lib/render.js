...
export function renderToolPage({ site, tool, related }) {
  ...
  const screenshotPath = `public/assets/tools/${tool.id}-social.png`;
  const hasScreenshot = fs.existsSync(path.join(process.cwd(), screenshotPath));
  ...
  const head = buildHead({
    ...
    path,
    type: 'website',
    image: hasScreenshot ? `/assets/tools/${tool.id}-social.png` : undefined,
