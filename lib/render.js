...
function renderControl(c) {
  ...
  <label for="${domId}">${esc(c.label)}</label>
  ...
  <label for="${domId}"><span>${esc(c.label)}</span></label>
</div>`;
    }
    case 'range': {
      return `<div class="field" data-field="${escAttr(c.id)}">
  <label for="${domId}">${esc(c.label)} <span class="range-val" data-range-value="${escAttr(c.id)}">${escAttr(c.value ?? c.min)}</span></label>
...
function renderOutput(o) {
  ...
  <label for="${domId}">${esc(o.label || 'Result')}</label>
  ...
  ${copyBtn('#' + domId)}
    ...
    case 'pre':
      return `<div class="output-wrap">
  <span class="field-label" id="${domId}-label">${esc(o.label || 'Result')}</span>
...
function renderActions(actions) {
  ...
  const buttons = actions.map((a) =>
    `<button type="button" class="btn ${a.primary ? 'btn-primary' : 'btn-ghost'}" data-action="${escAttr(a.id)}">${esc(a.label)}</button>`
  ).join('\n  ');
...
<article class="tool-page">
  <header class="tool-header">
    ...
    <div class="meta-row">
      <a class="chip" href="/category/${categorySlug(tool.category)}">${esc(tool.category)}</a>
      <span class="chip chip-free">Free · No sign-up</span>
      <span class="chip" title="Added to the collection">Added ${esc(tool.added)}</span>
    </div>
  </header>
...
${tool.examples.map((s) => `      <li>${inline(s)}</li>`).join('\n')}
  ...
  <section class="section" aria-labelledby="related-heading">
    <div class="section-head">
      ...
      <a href="/category/${categorySlug(tool.category)}">More ${esc(tool.category)} tools →</a>
    </div>
    ${toolGrid(related)}
  </section>
</article>`;
