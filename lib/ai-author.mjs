...
export function resolveProvider() {
  ...
  if (explicit === 'groq') return process.env.GROQ_API_KEY ? 'groq' : 'none';
  // auto
...
async function callGroqRaw({ system, user }, timeoutMs = 90000) {
  const maxAttempts = 4;
  let delay = 10000; // start with 10 seconds delay
    ...
    try {
      ...
      if (res.status === 429) {
        if (attempt === maxAttempts) {
          throw new Error(`Groq HTTP 429 (Rate Limit) after ${maxAttempts} attempts.`);
        }
        console.warn(`⏳ Groq HTTP 429 Rate Limit hit. Retrying in ${delay / 1000}s (attempt ${attempt}/${maxAttempts})...`);
        clearTimeout(timer);
      ...
      console.warn(`⚠️ Groq request attempt ${attempt} failed: ${err.message}. Retrying in ${delay / 1000}s...`);
      clearTimeout(timer);
      await new Promise(resolve => setTimeout(resolve, delay));
      delay *= 1.5;
    } finally {
