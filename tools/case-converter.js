(function () {
  ...
  var transforms = {};
  transforms["uppercase"] = function toUpper(text) {
  return text.toUpperCase();
};
  transforms["lowercase"] = function toLower(text) {
  return text.toLowerCase();
};
  transforms["title-case"] = function toTitleCase(text) {
  return text.toLowerCase().replace(/[\p{L}\p{N}][\p{L}\p{N}'’-]*/gu, function (word) {
    return word.charAt(0).toUpperCase() + word.slice(1);
  });
};
  transforms["sentence-case"] = function toSentenceCase(text) {
  var lower = text.toLowerCase();
  return lower.replace(/(^|[.!?]\s+|[\n]+\s*)([\p{L}])/gu, function (m, prefix, letter) {
    return prefix + letter.toUpperCase();
  });
};
  transforms["camel-case"] = function toCamelCase(text) {
  var words = text.toLowerCase().match(/[\p{L}\p{N}]+/gu) || [];
  return words.map(function (w, i) {
    return i === 0 ? w : w.charAt(0).toUpperCase() + w.slice(1);
  }).join('');
};
  transforms["pascal-case"] = function toPascalCase(text) {
  var words = text.toLowerCase().match(/[\p{L}\p{N}]+/gu) || [];
  return words.map(function (w) {
    return w.charAt(0).toUpperCase() + w.slice(1);
  }).join('');
};
  transforms["snake-case"] = function toSnakeCase(text) {
  var words = text.toLowerCase().match(/[\p{L}\p{N}]+/gu) || [];
  return words.join('_');
};
  transforms["kebab-case"] = function toKebabCase(text) {
  var words = text.toLowerCase().match(/[\p{L}\p{N}]+/gu) || [];
  return words.join('-');
};
  transforms["constant-case"] = function toConstantCase(text) {
