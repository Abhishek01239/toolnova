(function () {
  ...
  var transforms = {};
  transforms["remove-empty-lines"] = function removeEmptyLines(text) {
  return text.split('\n').filter(function (line) { return line.trim() !== ''; }).join('\n');
};
  transforms["trim-lines"] = function trimLines(text) {
  return text.split('\n').map(function (line) { return line.trim(); }).join('\n');
};
  transforms["remove-extra-spaces"] = function removeExtraSpaces(text) {
  return text.split('\n').map(function (line) {
    return line.replace(/[ \t]+/g, ' ').trim();
  }).join('\n');
};
  transforms["dedupe-lines"] = function dedupeLines(text) {
  var seen = new Set();
